...
**Service layer** (`src/services/`): Business logic for proof generation. Uses `@maticnetwork/maticjs` + ethers v5. Each chain has exactly one configured RPC endpoint (endpoint redundancy lives behind the endpoint, not in this service). Transient RPC failures get one bounded retry against the same endpoint (2 attempts total) with no delay between attempts; domain `HTTPError`s stop retrying immediately.
