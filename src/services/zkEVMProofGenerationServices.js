...
export async function bridge(networkID, depositCount, network) {
  ...
  const response = await fetch(`${zkEVMURL}/bridge?net_id=${networkID}&deposit_cnt=${depositCount}`);
...
export async function merkelProofGenerator(networkID, depositCount, network) {
  ...
  const response = await fetch(`${zkEVMURL}/merkle-proof?net_id=${networkID}&deposit_cnt=${depositCount}`);
  const data = await response.json();
