...
describe('matic exit payload — invalid arguments', { timeout: 60_000 }, () => {
  // burnTxHash without 0x prefix — wrong format
  it('should 400 with "Incorrect Burn tx or Event Signature!" for burnTxHash without 0x', async () => {
  ...
  it('should 400 with "Incorrect Burn tx or Event Signature!" for wrong-length hashes', async () => {
    const res = await getAgent().get(
  ...
  it('should 400 with "Invalid burnTxHash or eventSignature!" when eventSignature is missing', async () => {
  ...
  it('should 404 for valid args that have no on-chain exit data (tokenIndex=1)', async () => {
    const res = await getAgent().get(
